...
// Authors: Simon Davidson & Claude   Created: 2026-06-22   Last modified: 2026-07-21
//
...
// OVERRIDES (see the "PER-APPLICATION BUS SIZING" block and the acc_snn_processor
// instances below). u_snn0 is sized for a 1920-neuron RECURRENT layer — it
// EXCEEDS the legacy `PIN_BITS=10 (1024) cap purely by overriding
// NP_NEURON_IDX_SZ / SP_ACT_IDX_SZ, proving the parameterisation end-to-end in a
// full-system elaboration. Everything else mirrors flexman.v.
// =============================================================================
//
// Top-level integration module (example application).
...
// =============================================================================
// PER-APPLICATION BUS SIZING — the single source of truth for this example.
// Each accelerator instance below is sized for ITS largest layer from these
// localparams (change the network => change only these). Index widths come
// straight from neuron counts via $clog2; u_snn0's REC_IDX_SZ=11 EXCEEDS the
// legacy `PIN_BITS=10 (1024) cap purely by parameter override.
// =============================================================================
localparam REC_NEURONS = 1920;                 // u_snn0: recurrent layer  (>1024!)
localparam REC_IDX_SZ  = $clog2(REC_NEURONS);  // = 11
localparam ENC_NEURONS = 512;                  // u_snn1: encoder layer
localparam ENC_IDX_SZ  = $clog2(ENC_NEURONS);  // = 9

// ─── snnAcc0 (TGT_ACC_ID = 0) — RECURRENT layer, 1920 neurons (>1024) ────────
acc_snn_processor #(
    .TGT_ACC_ID           (0),
    .TGT_CONFIG_BASE_ADDR (SNN0_CFG_BASE),
    // ---- PER-INSTANCE SIZING: sized for REC_NEURONS=1920 (> legacy 1024 cap) ----
    // At the `PIN_BITS=10 default this instance would alias every neuron >=1024;
    // sizing the index buses to REC_IDX_SZ=11 lets one TASK cover the full layer.
    .NP_NEURON_IDX_SZ       (REC_IDX_SZ),   // output writeback (packer) index
    .SP_ACT_IDX_SZ          (REC_IDX_SZ),   // input-spike flat-fetch index
    .SP_SYN_CURR_IDX_SZ     (REC_IDX_SZ),
    .NP_SYN_CURR_IDX_SZ     (REC_IDX_SZ),
    .NP_POT_IDX_SZ          (REC_IDX_SZ),
    .NP_SPIKE_IDX_SZ        (REC_IDX_SZ),
...
acc_snn_processor #(
    ...
    // ---- PER-INSTANCE SIZING: ENCODER layer, ENC_NEURONS=512 (fits cap; states intent) ----
    .NP_NEURON_IDX_SZ       (ENC_IDX_SZ),   // = $clog2(512) = 9
    .SP_ACT_IDX_SZ          (ENC_IDX_SZ),
    .SP_SYN_CURR_IDX_SZ     (ENC_IDX_SZ),
    .NP_SYN_CURR_IDX_SZ     (ENC_IDX_SZ),
    .NP_POT_IDX_SZ          (ENC_IDX_SZ),
    .NP_SPIKE_IDX_SZ        (ENC_IDX_SZ),
    .NP_SYN_CURR_SLICE_BITS (32),
    .NP_POT_SLICE_BITS      (32),
    .NP_BIAS_CURR_SLICE_BITS(32),
    .SP_SYN_CURR_SLICE_BITS (32),
