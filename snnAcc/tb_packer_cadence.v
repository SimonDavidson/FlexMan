...
// =============================================================================
// tb_packer_cadence  (snnAcc)  --  dedicated multi-packer cadence test (F8)
//
...
// Purpose
// -------
// Close the snnAcc gap for F8 (verif/FINDINGS.md): "sub-32-bit slice writeback
// corrupts fields under WRITE-side back-pressure".  The canonical regression for
// this lived only in fmiSnnAcc/tb_packer_cadence.v; snnAcc's neuron_processing
// shares the identical writeback wiring (pak_write_i = result_taken + the F8
// guard in packer.v) yet had NO test that ever asserts a WRITE-side mem_wait
// (tb_neuron_processing ties all write waits to 0).  This reproduces snnAcc's
// neuron_processing writeback EXACTLY but in isolation:
//   * the REAL snnAcc update_state_for_neuron as producer (true result_valid /
//     result_taken / neuron_taken handshake and 3-cycle FSM cadence),
//   * a neuron counter that advances on result_taken (= neuron_taken),
//   * the REAL packer x3 wired identically to snnAcc/neuron_processing:
...
//     can inject back-pressure on the WRITE side.
//
...
//
// Golden: per packer, OR each neuron's left-justified field contribution into its
// destination word (the proven tb_packer field model).  This is timing- and
// cadence-INDEPENDENT: if the RTL packs each neuron into the correct word/field,
// memory matches regardless of how write waits perturb the schedule.  The neuron
// values themselves come from np_ref_lif (the shared LIF golden).
//
// Invariant: "exactly one accumulate per accepted neuron" (accepts == N) AND
// every written word == the field-model golden, across the full slice-size matrix
// under heavy write-back back-pressure.  The use_taken mux is retained (default 1
// = the FIXED wiring) so the as-built result_valid strobe can be re-checked by
// hand; check_eq_u prints a bare FAIL token that run_regression.sh greps, so a
// scored negative-control pass cannot coexist with a green run.
// =============================================================================
`timescale 1ns/1ps
...
module tb_packer_cadence;
    ...
    reg                      sub_on_fire;
    reg [`ADDR_SIZE-1:0]     syn_base, pot_base, spike_base;
    reg [1:0]                wmode;        // 0=no wait, 1=30% rand, 2=60% rand
    reg                      use_taken;    // 0 = as-built (result_valid strobe)
                                           // 1 = shipped fix (result_taken strobe)

    // per-neuron inputs (drive update_state)
    reg signed [31:0] in_syn[0:127], in_pot[0:127], in_bias[0:127], in_thr[0:127];
    reg        [31:0] in_dsyn[0:127], in_dpot[0:127];
    ...
    // -------------------------------------------------------------------------
    // Producer: REAL snnAcc update_state_for_neuron  (32-bit datapath)
    // -------------------------------------------------------------------------
    wire               result_valid, neuron_taken_unused, spike;
    wire signed [31:0] upd_pot, upd_syn;
    wire               packer_full, syn_wb_full, pot_wb_full;
    ...
    // snnAcc applies NO bin-point shift on the writeback path (updated_syn_curr is
    // just the sign-extended raw output), so this is a direct left-justify.
    // -------------------------------------------------------------------------
    ...
    wire [31:0] syn_curr_wb_lj = leftjust(upd_syn, syn_sz);
    wire [31:0] pot_wb_lj      = leftjust(upd_pot, pot_sz);

    // -------------------------------------------------------------------------
    // Writeback memory buses + REAL packers (wired as in snnAcc/neuron_processing)
    // -------------------------------------------------------------------------
    ...
    reg                   spike_wait, syn_wait, pot_wait;

    // Strobe under test: as-built drives the packers from the HELD result_valid
    // level; the shipped fix drives them from the one-cycle result_taken accept.
    ...
    packer #(.PAK_IDX_SZ(NEURON_IDX_SZ)) spike_packer0 (
        ...
        .pak_index_i(neuron_counter_r), .pak_acc_data_i({spike, {(`POT_BITS-1){1'b0}}}),
    ...
    packer #(.PAK_IDX_SZ(NEURON_IDX_SZ)) syn_curr_wb_packer (
        ...
        .pak_colour_i(1'b0), .pak_last_i(last_neuron),
        .pak_index_i(neuron_counter_r), .pak_acc_data_i(syn_curr_wb_lj),
    ...
    packer #(.PAK_IDX_SZ(NEURON_IDX_SZ)) pot_wb_packer (
        ...
        .pak_colour_i(1'b0), .pak_last_i(last_neuron),
        .pak_index_i(neuron_counter_r), .pak_acc_data_i(pot_wb_lj),
    ...
    // -------------------------------------------------------------------------
    reg [31:0] spk_sram[0:255], syn_sram[0:255], pot_sram[0:255];
    always @(posedge clk) begin
    ...
    always @(posedge clk) begin
        case (wmode)
            ...
            default: begin
                spike_wait <= 1'b0; syn_wait <= 1'b0; pot_wait <= 1'b0;
            end
        endcase
    end
    ...
    reg [31:0] e_spk[0:255], e_syn[0:255], e_pot[0:255];
    ...
    task run_block;
        ...
        begin
            ...
            for (sram_i=0; sram_i<256; sram_i=sram_i+1) begin
                spk_sram[sram_i]=0; syn_sram[sram_i]=0; pot_sram[sram_i]=0;
                e_spk[sram_i]=0;    e_syn[sram_i]=0;    e_pot[sram_i]=0;
            end
            ...
            for (n = 0; n < N; n = n + 1) begin
                ...
                np_ref_lif(in_syn[n], in_pot[n], in_bias[n], in_thr[n],
                           in_dsyn[n], in_dpot[n], sof,
            ...
            last_neuron_idx = N - 1;
            syn_sz = ssz; pot_sz = psz; sub_on_fire = sof; wmode = wm;
            ...
            // was full+stalled and the next neuron's strobe re-OR'd in.  Drop the
            // write-back wait now and drain generously so every buffer flushes its
            // FINAL (possibly corrupted) word for inspection.
            wmode = 2'd0;
    ...
    initial begin
        ...
        syn_sz=SZ32; pot_sz=SZ32; sub_on_fire=0; wmode=0;
        spike_wait=0; syn_wait=0; pot_wait=0;
        syn_base=20; pot_base=50; spike_base=80;
        reset=1; repeat(2) @(posedge clk); #1; reset=0; @(posedge clk); #1;

        $display("=== tb_packer_cadence (snnAcc) ===");
        void'($urandom(32'h5A11_0F8));

        // Shipped wiring: pak_write_i = result_taken (one-cycle accept) + the
        // packer.v F8 guard.  Exercised across the full slice-size matrix under
        // heavy write-back back-pressure.  The as-built (result_valid) negative
        // control that originally demonstrated F8 is documented in
        // verif/FINDINGS.md and not scored here (check_eq_u prints a bare FAIL).
        use_taken = 1'b1;
        ...
        run_block(33, 1'b1, SZ32, SZ32, 2'd2, "C3 32b subFire heavyWait");
        ...
        run_block(33, 1'b1, SZ16, SZ16, 2'd2, "S4 16b subFire wait60");
