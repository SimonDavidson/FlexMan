...
module act_index_generator
		           ...
		           parameter CIN_SZ             = 7,             // MC: input-channel index width
   ...
   (
    ...
    output wire          [CIN_SZ-1:0] act_index_cin_o,     // MC: current input channel
...
reg                    [CIN_SZ-1:0] cin_index_r;          // MC: input-channel counter
...
// MC: input-channel counter (advances when x & y both wrap).
// Backwards-compatible: with cin_len_i==1, cin_at_end is always true and
// cin_index_r stays at 0, so act_index_last_o matches the original behaviour.
assign cin_at_end    = (cin_index_r == (cin_len_i - 1'b1));
assign xy_wrap       = act_index_taken_i & in_x_end_of_row & in_y_end_of_col;
assign cin_index_nxt = (xy_wrap & cin_at_end) ? {CIN_SZ{1'b0}} :
                       (xy_wrap)              ? (cin_index_r + 1'b1) :
                                                 cin_index_r;

// Count through the input image, in 2-D, sweeping channels in the outermost loop.
always @ (posedge clk)
   if (reset | ~act_index_gen_running_r)
      begin
         in_x_index_r    <= 1'b0;
         in_y_index_r    <= 1'b0;
         cin_index_r     <= {CIN_SZ{1'b0}};
         in_elem_count_r <=  'b0;
      end
   else
      begin
