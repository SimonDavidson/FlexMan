...
// =============================================================================
// tb_acc_fmiSnnMC_stress  (fmiSnnAccMC)  --  focused F9 gapped-activation check
//
// Authors      : Simon Davidson & Claude
// Created      : 2026-06-30
// Last modified: 2026-06-30
//
// Port of snnAcc/tb_acc_snn_stress.v narrowed to the F9 surface for fmiSnnAccMC
// (top `acc_fmiSnnMC_processor`): confirm the full-mode FC weight-cache phase fix
// (the `& act_data_valid_i` gate on weight_index_valid_full) holds end-to-end.
//
// Runs the WHOLE acc_fmiSnnMC_processor with sp_skip_neuron=1, single channel
// (cin=cout=1) and legacy spike mode (real_mac off), so it only exercises
// spike_processing's accumulation. 1-bit SPIKE gate (active = act MSB set), 1-D.
// We drive act in {0,1} (1 = spike -> contributes weight; 0 = GAP), so the
// connectivity golden is syn[j] = sum over spiking i of weight[i][j]. The 9
// NP-side ports are tied off. FULL + SPARSE only (the F9 surface): the MC's conv
// is a native multi-channel mode covered by the MC's own integration tests.
//
...
module tb_acc_fmiSnnMC_stress;
    ...
    localparam [31:0]  S2_1CHAN    = 32'h0101_0401;   // cout=1|cin=1|stride=1|ts=1
    localparam         IS_MAC      = 1'b0;            // legacy 1-bit spike gate
    ...
    acc_fmiSnnMC_processor # (
        ...
        .SP_ACT_SLICE_SZ(`ACT_SLICE_SZ), .SP_ACT_IDX_SZ(`ACT_IDX_SZ), .SP_ACT_DATA_IDX_SZ(`ACT_DATA_IDX_SZ),
    ...
    // ================================================================
    // Per-config state + connectivity golden (full / sparse, 1-D)
    // ================================================================
    ...
    task configure;
        begin
            ...
            cfg_write(CFG+32'h08, SYN_BASE);     cfg_write(CFG+32'h0C, 100);
            cfg_write(CFG+32'h10, 110);          cfg_write(CFG+32'h14, 120);
            cfg_write(CFG+32'h18, 130);          cfg_write(CFG+32'h1C, 140);
            cfg_write(CFG+32'h20, 150);          cfg_write(CFG+32'h24, 160);
            cfg_write(CFG+32'h28, 170);          cfg_write(CFG+32'h2C, 180);
            cfg_write(CFG+32'h30, (coutx<<16) | cinx);        // S0 out_x | in_x
            cfg_write(CFG+32'h34, ((nout-1)<<16) | crpn);     // S1 last_neuron | rpn
            cfg_write(CFG+32'h38, S2_1CHAN);                  // S2 cin=cout=1, stride=1, ts=1
            // M0: skip=1 | wpw=4 | weight_sz | syn_sz=5 | pot_sz=5 | weight_mode | has_ada=0
            cfg_write(CFG+32'h3C, 32'd1 | (4<<6) | (cwszc<<16) | (5<<20) | (5<<24) | (cmode<<28));
            cfg_write(CFG+32'h5C, 32'd5);
            cfg_write(CFG+32'h74, 32'd1); cfg_write(CFG+32'h7C, 32'd1); cfg_write(CFG+32'h84, 32'd0);
    ...
    task scn_sparse;
        ...
        begin
            ...
            ciszc=iszc; ctszc=tszc; cscnt=scnt;
            rand_acts;
    ...
    task gap_probe;
        begin
            ...
            cinx=4; coutx=4; nin=4; nout=4; crpn=1; ciszc=0;ctszc=0;cscnt=0;
    ...
    initial begin
        ...
        $display("=== tb_acc_fmiSnnMC_stress (fmiSnnAccMC full-chip, skip_neuron, 1-chan legacy spike) ===");
        void'($urandom(32'hFC_5417));
        ...
        `VERIF_EPILOGUE("tb_acc_fmiSnnMC_stress")
    end
