...
// critical combinatorial path (mul -> mux -> packer barrel shifter ->
// output_buffer) that failed 200 MHz timing.  Functionally identical to the
// prior 2-cycle version; ported from snnAcc/update_state_for_neuron.v (which
// already closes 200 MHz).  The valid/taken handshake absorbs the +1 cycle, so
// neuron_processing needs no change.
//
...
module update_state_for_neuron # (
    ...
    // F1 fix (2026-06-07, ported here 2026-06-10): signed*signed decay. The
    // bare `mul_a * mul_b` was evaluated UNSIGNED (mul_b is unsigned),
    // corrupting decay of negative operands — RTL stored EMU_value + mult for
    // every negative syn/pot (caught by the e2e frame-1 encoder divergence).
    // Zero-extend mul_b and cast both signed, matching snnAcc/annAcc/fmiSnnAcc.
