...
// =============================================================================
// tb_acc_ipsnn_stress  (ipSnnAcc)  --  focused F9 gapped-activation confirmation
//
// Authors      : Simon Davidson & Claude
// Created      : 2026-06-30
// Last modified: 2026-06-30
//
// Port of snnAcc/tb_acc_snn_stress.v narrowed to the F9 surface: confirm that the
// full-mode FC weight-cache phase fix (the `& act_data_valid_i` gate on
// weight_index_valid_full, commit 8e672ac) holds end-to-end in ipSnnAcc, where a
// mid-grid GAPPED (inactive) input used to make the next active input read the
// previous input's weight row.
//
// Runs the WHOLE acc_snn_processor with sp_skip_neuron=1 (neuron stage off), so it
// only exercises spike_processing's accumulation -- no neuron-model golden needed.
// ipSnnAcc is a MAC front-end: active input = act_value != 0, syn += act_value x
// weight (gate = act!=0; act here is 8-bit, 4/word). We drive act in {0,1} (1 =
// active -> contributes the weight; 0 = GAP), so the connectivity golden is
// syn[j] = sum over active i of weight[i][j] -- and gaps probe F9.
//
// Two oracles per config: (1) software connectivity golden (full/sparse/conv
// routing built consistent-by-construction); (2) stall-invariance -- the same
// config re-run with heavy per-port stalls on act/weight/syn_curr must reproduce
// the no-stall syn_curr byte-for-byte. The directed gap_probe is a scored guard.
// =============================================================================
`timescale 1ns/1ps

`include "../shared/constants.v"

// CONFIG PARAMS START (compile-time RTL params; ipSnnAcc acc_snn_processor is
// byte-identical to snnAcc's, so the param/port map is shared. WEIGHT_SLICE_SZ=5
// => 32-bit weight bus covers every runtime weight_sz and a full sparse tuple.
// ACT_SLICE_SZ=3 => ipSnn 8-bit MAC activations, 4 per word.)
...
module tb_acc_ipsnn_stress;
    ...
    localparam         IS_MAC      = 1'b1;     // ipSnnAcc: 8-bit MAC activations
    ...
    // ---- active SP-side memory buses (driven by BFMs) ----
    ...
    wire [`WTD_BITS-1:0] bias_curr_mem_data_i = 0, thresh_mem_data_i = 0;
    wire [`POT_BITS-1:0] pot_mem_data_i = 0;
    ...
    .MEM_ADDR_BITS(`ADDR_SIZE)
        ...
        .bias_curr_mem_rd_o(bias_curr_mem_rd_o), .bias_curr_mem_wait_i(1'b0),
    ...
    // ---- wait-honouring BFMs on the three SP-side ports ----
    ...
    // ---- stall generator (heavier on syn to stress the arbiter path) ----
    reg     stall_en;
    integer stall_pct;
    ...
    task pack_stream;                 // weight element k (wb-bit) at stream bit wb*k
    ...
    // ================================================================
    // Per-config state + connectivity golden (skip_neuron: syn = accumulation)
    // ================================================================
    ...
    integer           act      [0:31];      // per-input multiplier (0 = GAP)
                    ...
                    if (act[gi] != 0)
                        for (goy=0; goy<ckyl; goy=goy+1)
                          for (gox=0; gox<ckxl; gox=gox+1) begin
                             gk = (gi/cinx)*ckys + goy - ckyo;
                             gj = (gi%cinx)*ckxs + gox - ckxo;
                             if (gj>=0 && gj<coutx && gk>=0 && gk<couty)
                                syn_acc[gk*coutx+gj] = syn_acc[gk*coutx+gj] + act[gi]*kern[goy*ckxl+gox];
                          end
            end
        end
    endtask
    ...
    task setup_mem;
        ...
        begin
            ...
            // activations: MAC variant packs an 8-bit value/input (4/word); a gate
            // variant would pack 1 bit/input. ipSnnAcc is MAC.
            for (i=0; i<nin; i=i+1)
            ...
            for (i=0; i<nout; i=i+1) syn_sram[SYN_BASE + i] = 0;   // accumulate from 0
        end
    endtask
    ...
    task configure;
        begin
            ...
            cfg_write(CFG+32'h08, SYN_BASE);     cfg_write(CFG+32'h0C, 30);
            cfg_write(CFG+32'h10, 40);           cfg_write(CFG+32'h14, 50);
            cfg_write(CFG+32'h18, 60);           cfg_write(CFG+32'h1C, 32'h8000_0000);
            cfg_write(CFG+32'h20, 32'h8000_0000);
            cfg_write(CFG+32'h24, (ciny<<16) | cinx);
            cfg_write(CFG+32'h28, (couty<<16) | coutx);
            cfg_write(CFG+32'h2C, ((nout-1)<<16) | crpn);
            cfg_write(CFG+32'h30, 32'd1);
            cfg_write(CFG+32'h34, cwszc | (5<<4) | (3<<8) | (5<<12) | (cmode<<24));
            cfg_write(CFG+32'h38, 32'd1 | (4<<6));        // sp_skip_neuron=1, weights_per_word=4
    ...
    // ================================================================
    // Randomised config builders (gapped activations)
    // ================================================================
    integer i, t;
    task rand_acts;                  // ~75% active, value 1 (0 = GAP) -> probes F9
    ...
    task scn_full;
        ...
        begin
            ...
            cinx=2; ciny=2; coutx=4; couty=1; nin=4; nout=4;
    ...
    task scn_sparse;
        ...
        begin
            ...
            ciszc=iszc; ctszc=tszc; cscnt=scnt; ckxl=1;ckyl=1;ckxs=1;ckys=1;ckxo=0;ckyo=0;
            rand_acts;
    ...
    task scn_conv;
        ...
        input integer kxl,kyl,kxs,kys,kxo,kyo, inx,iny,outx,outy; input [255:0] tag;
        begin
    ...
    // Directed F9 guard: mid-grid GAP, distinct known weights, act=1 for active.
    integer pj;
    task gap_probe;
        begin
            ...
            for (pj=0;pj<nout;pj=pj+1)            // must read grid rows {0,1,3} -> 70,73,76,79
    ...
    initial begin
        ...
        $display("=== tb_acc_ipsnn_stress (ipSnnAcc full-chip, skip_neuron, MAC) ===");
        void'($urandom(32'h19_5417));
        ...
        for (rep=0; rep<3; rep=rep+1) begin
            ...
            scn_conv(8, 3'b011, 1,1,1,1,0,0, 3,3,3,3, "C_1x1");
            scn_conv(8, 3'b011, 3,3,1,1,1,1, 3,3,3,3, "C_3x3");
            scn_conv(8, 3'b011, 3,1,2,1,1,0, 4,1,2,1, "C_strd2");
        end
        gap_probe;

        `VERIF_EPILOGUE("tb_acc_ipsnn_stress")
    end
