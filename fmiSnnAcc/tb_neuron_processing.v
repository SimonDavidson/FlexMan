...
module tb_neuron_processing;
    ...
    // path: a held writeback that, on as-built RTL, lets the next neuron re-OR
    // into a full/already-flushed word). Spike held heavier to stress the 1-bit
    // packer's word-boundary / final flush.
    reg bp_en;
    ...
    task run_block;
        ...
        begin
            ...
            // Drop back-pressure FIRST, then drain: neuron_proc_finished leads the
            // final packer writeback (it pulses on last-neuron ACCEPT), and under
            // write back-pressure that final flush can still be stalled. Any F8
            // corruption already happened during the run; draining wait-free just
            // lets the final (possibly corrupted) word commit for inspection.
            bp_en = 0;
