...
// =============================================================================
// tb_acc_ann_stress  (annAcc)  --  focused F9 gapped-activation confirmation
//
// Authors      : Simon Davidson & Claude
// Created      : 2026-06-30
// Last modified: 2026-06-30
//
// Port of snnAcc/tb_acc_snn_stress.v narrowed to the F9 surface for annAcc (top
// module `ann_processor`): confirm the full-mode FC weight-cache phase fix (the
// `& act_data_valid_i` gate on weight_index_valid_full) holds end-to-end where a
// mid-grid GAPPED input used to make the next active input read the previous
// input's weight row.
//
// Runs the WHOLE ann_processor with sp_skip_neuron=1 (neuron stage off, no ANN
// neuron-model golden needed) and np_bias_en=0 (no bias seed), so it only
// exercises spike_processing's MAC accumulation. annAcc is an 8-bit MAC front-end
// (active = act!=0, syn += act x weight); act runtime width = M0[19:16]=3 (8-bit).
// We drive act in {0,1} (1 = active -> contributes the weight; 0 = GAP), so the
// connectivity golden is syn[j] = sum over active i of weight[i][j].
//
...
// CONFIG PARAMS START (annAcc ann_processor param map; ACT_SLICE_SZ=5 => 32-bit
// act bus, runtime element width via M0[19:16]; NP_LUT_* replace bias params.)
...
module tb_acc_ann_stress;
    ...
    localparam         IS_MAC      = 1'b1;     // annAcc: 8-bit MAC activations
    ...
    .NP_LUT_IDX_SZ(`NP_LUT_IDX_SZ), .NP_LUT_DATA_IDX_SZ(`NP_LUT_DATA_IDX_SZ),
    .NP_LUT_SLICE_SZ(`NP_LUT_SLICE_SZ), .NP_LUT_SLICE_BITS(`NP_LUT_SLICE_BITS),
    ...
    // ================================================================
    // Per-config state + connectivity golden
    // ================================================================
    ...
    task configure;
        begin
            ...
            // annAcc M0: weight_sz | syn_curr_sz=5 | lut_out_sz=3 | pot_sz=5 |
            //            act_sz=3(8-bit) | thresh_op=0 | weight_mode. No flags (act
            //            unsigned, np_bias_en=0 -> pure MAC, no bias seed).
            cfg_write(CFG+32'h34, cwszc | (5<<4) | (3<<8) | (5<<12) | (3<<16) | (cmode<<24));
    ...
    task gap_probe;
        begin
            ...
            ckxl=1;ckyl=1;ckxs=1;ckys=1;ckxo=0;ckyo=0; ciszc=0;ctszc=0;cscnt=0;
            act[0]=1; act[1]=1; act[2]=0; act[3]=1;
    ...
    initial begin
        ...
        $display("=== tb_acc_ann_stress (annAcc full-chip, skip_neuron, MAC) ===");
        void'($urandom(32'h0A_5417));
        ...
        `VERIF_EPILOGUE("tb_acc_ann_stress")
    end
